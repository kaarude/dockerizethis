...
func TestHealthRoutes(t *testing.T) {
	for _, tc := range []struct{ name, code, file, want string }{
		{"exact GET", `app.MapGet("/healthz", () => "ok");`, "Program.cs", "/healthz"},
		{"POST", `app.MapPost("/health", () => "ok");`, "Program.cs", ""},
		{"comment", `// app.MapGet("/health", () => "ok");`, "Program.cs", ""},
		{"block comment", `/* app.MapGet("/health", () => "ok"); */`, "Program.cs", ""},
		{"string", `var example = "app.MapGet(\"/health\", handler)";`, "Program.cs", ""},
		{"raw string", `var example = """app.MapGet("/health", handler)""";`, "Program.cs", ""},
		{"group", `var group = app.MapGroup("/api"); group.MapGet("/health", () => "ok");`, "Program.cs", ""},
		{"base path", `app.UsePathBase("/api"); app.MapGet("/health", () => "ok");`, "Program.cs", ""},
		{"test attribute", `[Fact] public void Test() { app.MapGet("/health", () => "ok"); }`, "Program.cs", ""},
		{"attribute", `[Route("health")] class Health {}`, "Program.cs", ""},
		{"test source", `app.MapGet("/health", () => "ok");`, "Tests/Health.cs", ""},
	} {
		t.Run(tc.name, func(t *testing.T) {
			dir := t.TempDir()
			write(t, dir, "app.csproj", `<Project Sdk="Microsoft.NET.Sdk.Web"><PropertyGroup><TargetFramework>net8.0</TargetFramework></PropertyGroup></Project>`)
			write(t, dir, tc.file, "var builder = WebApplication.CreateBuilder(args);\nvar app = builder.Build();\n"+tc.code)
			p, ok, err := (dotnet.Detector{}).Detect(dir)
			require.NoError(t, err)
			require.True(t, ok)
...
func TestNestedProjectSettingsAndScope(t *testing.T) {
	dir := t.TempDir()
	write(t, dir, "App.sln", `Project("SDK") = "Api", "src/Api/Api.csproj", "ID"`)
	write(t, dir, "src/Api/Api.csproj", `<Project Sdk="Microsoft.NET.Sdk.Web"><PropertyGroup><TargetFramework>net8.0</TargetFramework></PropertyGroup></Project>`)
	write(t, dir, "src/Api/Properties/launchSettings.json", `{"profiles":{"http":{"applicationUrl":"http://localhost:5217"}}}`)
	write(t, dir, "Unrelated/Program.cs", `var app = WebApplication.CreateBuilder(args).Build(); app.MapGet("/health", () => "ok");`)
	p, ok, err := (dotnet.Detector{}).Detect(dir)
	require.NoError(t, err)
	require.True(t, ok)
...
func TestUnsupportedExecutables(t *testing.T) {
	for _, properties := range []string{
		`<TargetFramework>net472</TargetFramework><OutputType>Exe</OutputType>`,
		...
		dir := t.TempDir()
		write(t, dir, "app.csproj", `<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup>`+properties+`</PropertyGroup></Project>`)
		p, ok, err := (dotnet.Detector{}).Detect(dir)
		require.NoError(t, err)
		require.True(t, ok)
...
func TestSolutionCannotReadOutsideContext(t *testing.T) {
	for _, name := range []string{"../outside.csproj", "/outside.csproj"} {
		dir := t.TempDir()
		write(t, dir, "App.sln", `Project("SDK") = "Api", "`+name+`", "ID"`)
		_, ok, err := (dotnet.Detector{}).Detect(dir)
		require.ErrorContains(t, err, "inside the build context")
		require.False(t, ok)
	}
	dir := t.TempDir()
	outside := t.TempDir()
	write(t, outside, "app.csproj", `<Project />`)
	require.NoError(t, os.Symlink(filepath.Join(outside, "app.csproj"), filepath.Join(dir, "app.csproj")))
	_, ok, err := (dotnet.Detector{}).Detect(dir)
	require.Error(t, err)
	require.False(t, ok)
}
