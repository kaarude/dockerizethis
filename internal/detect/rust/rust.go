...
var (
	sectionPattern   = regexp.MustCompile(`^\s*\[\[?([A-Za-z0-9_.'"()-]+)\]?\]`)
...
type manifest struct {
	...
	binName     string // last [[bin]] name wins, matching Cargo
...
func parseManifest(source string) manifest {
	...
	for _, line := range strings.Split(stripComments(source), "\n") {
		...
		case "bin":
			if key == "name" {
				m.binName = literal(value)
			}
		case "dependencies", "build-dependencies":
...
func (Detector) Detect(dir string) (plan.Plan, bool, error) {
	...
	if m.hasPackage {
		...
		noBinTarget = m.binName == "" && !binTargetExists(dir)
	} else {
		p.Confidence = 0.4
		p.Notes = append(p.Notes, "workspace root without [package]; set Extras[\"binPackage\"] and Extras[\"bin\"] to the member and binary to build")
	}

	deps := map[string]string{}
	for _, d := range m.deps {
		if _, seen := deps[d.name]; !seen {
			deps[d.name] = d.spec
		}
	}
	...
	if noBinTarget {
		...
		p.Notes = append(p.Notes, "no src/main.rs, src/bin, or [[bin]] target; confirm the binary this package builds")
	}
...
// binTargetExists reports whether the package has a conventional binary
// target: src/main.rs or any entry under src/bin/.
func binTargetExists(dir string) bool {
...
var (
	...
	healthAttrPattern = regexp.MustCompile(`#\[(?:get|post|route)\s*\(\s*"(/health)`)
	healthCallPattern = regexp.MustCompile(`\.(?:route|at|service|nest)\s*\(\s*"(/?health)`)
)
...
func scanSources(dir string, loosePorts bool) (map[string]bool, int, string, error) {
	...
	err := filepath.WalkDir(dir, func(filename string, entry fs.DirEntry, walkErr error) error {
		...
		if health == "" {
			for _, pattern := range []*regexp.Regexp{healthAttrPattern, healthCallPattern} {
				if match := pattern.FindSubmatch(data); match != nil {
					health = "/health"
					break
				}
