...
import (
	...
	renderer "github.com/carl/dockerizethis/internal/templates/rust"
	"github.com/stretchr/testify/require"
)

// Run with DOCKERIZETHIS_DOCKER_TEST=1 go test ./internal/templates/rust -run TestDocker -v.
func TestDocker(t *testing.T) {
	if os.Getenv("DOCKERIZETHIS_DOCKER_TEST") != "1" {
		t.Skip("set DOCKERIZETHIS_DOCKER_TEST=1 to build and run generated images")
	}
	for _, fixture := range []string{"rust-web", "rust-worker", "rust-bin-worker"} {
		t.Run(fixture, func(t *testing.T) {
			...
			require.True(t, ok)
			files, err := renderer.RenderRust(p)
			require.NoError(t, err)
			for _, file := range files {
				require.NoError(t, os.WriteFile(filepath.Join(dir, file.Path), file.Content, file.Mode))
			}
			image := fmt.Sprintf("dockerizethis-rust-test-%d", time.Now().UnixNano())
