...
// on gradle:jdkVERSION into build/libs/. Quarkus copies its complete
// quarkus-app distribution instead. The runtime is always eclipse-temurin:VERSION-jre.
func RenderJava(p plan.Plan) ([]emit.File, error) {
	...
	if strings.TrimSpace(p.BuildCmd) == "" || strings.ContainsAny(p.BuildCmd, "\r\n\x00") || strings.HasSuffix(strings.TrimSpace(p.BuildCmd), "\\") {
	...
	case "maven":
		...
		if p.Framework == "quarkus" {
			d.QuarkusDir = "target/quarkus-app"
		}
	case "gradle":
		d.BuildImage = "gradle:jdk" + p.Version
		d.JarGlob = "build/libs/*.jar"
		if p.Framework == "quarkus" {
			d.QuarkusDir = "build/quarkus-app"
		}
	default:
