...
func TestRenderJavaGolden(t *testing.T) {
	for _, tc := range []struct {
		...
		{"quarkus", func(p *plan.Plan) { p.Framework = "quarkus"; p.StartCmd = "java -jar /app/quarkus-app/quarkus-run.jar" }},
	} {
...
func TestQuarkusPackaging(t *testing.T) {
	for _, tc := range []struct{ manager, dir string }{{"maven", "target"}, {"gradle", "build"}} {
		p := webPlan()
		p.PkgManager = tc.manager
		p.Framework = "quarkus"
		p.StartCmd = "java -jar /app/quarkus-app/quarkus-run.jar"
		files, err := java.RenderJava(p)
		require.NoError(t, err)
		dockerfile := string(files[0].Content)
		require.Contains(t, dockerfile, "/app/"+tc.dir+"/quarkus-app/ /app/quarkus-app/")
		require.Contains(t, dockerfile, "exec java -jar /app/quarkus-app/quarkus-run.jar")
		require.NotContains(t, dockerfile, "RUN cp ")
	}
	p := webPlan()
	p.BuildCmd = ""
	_, err := java.RenderJava(p)
	require.ErrorContains(t, err, "build command")
}
