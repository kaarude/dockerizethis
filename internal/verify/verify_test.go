...
func TestSmokeHTTPAndCleanup(t *testing.T) {
	for _, tc := range []struct {
		...
		{"health 404 fails", "/health", "", 404, 3},
		{"health 500 fails", "/health", "", 500, 3},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			p := plan.Plan{Process: plan.ProcessWeb, Port: 8080, HealthPath: tc.path, Extras: map[string]string{"verifyImage": "dockerizethis-verify:test"}}
...
func TestSmokeRejectsInvalidPlanWithoutDocker(t *testing.T) {
	...
	for _, p := range []plan.Plan{
		...
		{Process: plan.ProcessWeb, Port: 8080},
		{Process: plan.ProcessWeb, Port: 8080, HealthPath: "//outside/", Extras: map[string]string{"verifyImage": "test"}},
		{Process: plan.ProcessWeb, Port: 8080, HealthPath: "/%zz", Extras: map[string]string{"verifyImage": "test"}},
	} {
		_, err := Smoke(context.Background(), t.TempDir(), p)
...
func TestSmokeDoesNotFollowRedirects(t *testing.T) {
	...
	defer server.Close()
	t.Setenv("DOCKER_TEST_ADDRESS", strings.TrimPrefix(server.URL, "http://"))
	_, err := Smoke(context.Background(), t.TempDir(), plan.Plan{Process: plan.ProcessWeb, Port: 8080, HealthPath: "/health", Extras: map[string]string{"verifyImage": "test"}})
	require.NoError(t, err)
